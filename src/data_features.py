...
def build_features(df: pl.DataFrame, config: Dict) -> Tuple[pl.DataFrame, pl.Series]:
    #Unpack config
...
def to_samples(ts: np.ndarray, n_in: int = 90, n_out: int = 1) -> Tuple[np.ndarray, np.ndarray]:
...
def handle_holidays(df: pl.DataFrame, target: str = "TOTAL_SOLD", threshold: int =-2100,
